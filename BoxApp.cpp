...
void LitColumnsApp::Draw(const GameTimer& gt)
{
	...
	auto a = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
		D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
	mCommandList->ResourceBarrier(1, &a);
	...
	auto b = CurrentBackBufferView();
	auto c = DepthStencilView();
	mCommandList->OMSetRenderTargets(1, &b, true, &c);
	...
	// Indicate a state transition on the resource usage.
	auto d = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
		D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
	mCommandList->ResourceBarrier(1, &d);
...
void LitColumnsApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\colorVS.hlsl", nullptr, "main", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders\\colorPS.hlsl", nullptr, "main", "ps_5_1");
...
void LitColumnsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	{
		auto ri = ritems[i];
		auto a = ri->Geo->VertexBufferView();
		auto b = ri->Geo->IndexBufferView();
		cmdList->IASetVertexBuffers(0, 1, &a);
		cmdList->IASetIndexBuffer(&b);
